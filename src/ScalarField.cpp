...
namespace radiopropa {
    ...
    CloudModel_atm::CloudModel_atm(double _z_bottom, double _z_top, double _T0, double _p0, double _e) : z_bottom(
            _z_bottom), z_top(_z_top), T0(_T0), p0(_p0), e(_e) {
    }
    ...
    double CloudModel_atm::D = 6.1121;
    ...
    double CloudModel_atm::getValue(const Vector3d &position) const {
        ...
        if (z_bottom < position.z && position.z < z_top) {
            double N = 77.6 * P_air / T - 5.6 * P_sat/T + 3.75 * 1e5 * P_sat / pow(T, 2);
            return N * 1e-6 + 1.0;
        } else {
    ...
    Vector3d CloudModel_atm::getGradient(const Vector3d &position) const {
        ...
        if (z_bottom < position.z && position.z < z_top) {
            double dN = 77.6 * (1 / T * dP_air - P_air / pow(T, 2) * dT) - 5.6 *(1/T*dP_sat - 1/pow(T,2)*P_sat*dT) +
                        3.75 * 1e5 * (1 / pow(T, 2) * dP_sat - 2 * P_sat / pow(T, 3) * dT);
            double dn = dN * 1e-6;
